...
function updateHighscoreDisplay() {
    ...
    while (highscores.length < maxTopEntries) { // wenn nötig, die highscores mit platzhaltern auffüllen 
...
function isNewHighscore(score) {
    ...
    if (score > 0) {    // ist überhaupt ein punkt erzielt worden?
        if (highscores.length < maxTopEntries) { // ist noch platz in der liste?
            return true;
        } else if (score > highscores[highscores.length - 1].score) { // ist besser als letzt­plat­zierte?
            return true;
        }
    }
